...
async def auto_filter(bot: Client, update: Message):
    ...
    if len(results) == 0: # double check
        ...
        if len_result != 1:
            result[0].append(
                [
                    InlineKeyboardButton("Tap Here FOR NEXT ⏩", callback_data=f"navigate(0|next|{query})")
                ]
            )
        ...
        try:
            await bot.send_message(
                ...
                text=f"For any details check @lyf_of_chaos ⚜..Found {(len_results)} Results For your request: <code>{query}</code>",
